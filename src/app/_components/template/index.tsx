          ...
          <div className="mt-4 flex flex-col gap-3">
            ...
            <div className="mt-4 flex items-center gap-2 flex-wrap">
              {portfolio?.contacts?.map((item, index) => {
                const exists = SOCIAIS.find(
                  (item_) => Social[item_.type] == item.type
                );
                return (
                  <a
                    key={index}
          ...
          <section id="projects" className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-4">
              {portfolio?.projects?.map((item, index) => (
                <article
                  key={index}
                  className="relative w-full flex flex-col items-start px-4 pt-8 pb-4 border shadow-sm rounded-lg transition duration-300 hover:shadow-lg"
                >
                  ...
                  <div className="w-full flex flex-col gap-1">
                    ...
                    <div className="w-full flex items-center flex-wrap gap-2">
                      ...
                      {item?.techs.split(",").map((item) => (
                        <span
                          key={item}
                          className="bg-gray-200 hover:bg-gray-300 py-1 px-2 rounded-lg text-sm"
                        >
                          {item}
                        </span>
                      ))}
            ...
            {portfolio?.experiences?.map((item, index) => (
              <div key={index} className="mb-6 p-4 border rounded-lg shadow-sm">
                ...
                <p className="text-gray-700 mb-2 flex items-center gap-2">
                  <IoBusiness /> {item.company} - {item.location}
                </p>
            ...
            {portfolio?.certifications?.map((item, index) => (
              <div key={index} className="mb-6 p-4 border rounded-lg shadow-sm">
                ...
                <p className="text-gray-700">
                  {item.provider} - {new Date(item.date).getFullYear()}
                </p>
              </div>
            ))}
          </section>
